...
- **Fase 3 (hecha):** mismo módulo **Competencias** para **padre en nube** en **solo lectura**: entrada en menú ☰ junto a Recursos, acceso desde Inicio si el atajo aplica, lista y detalle (partidos / tabla / inscripciones) **sin** crear competencia, registrar partido, inscribir equipo ni editar resultado (`AcademiaNavPolicy`, `AcademiaRoot`, `CompetenciasScreen`, `strings.xml`). La visibilidad de datos sigue gobernada por **RLS** en Supabase.
